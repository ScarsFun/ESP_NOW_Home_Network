...
void send_mqtt_ota_status (char *status, char *node_id)
{
    MQTT_t mqttBuf;
    mqttBuf.topic_type = PUBLISH;
    sprintf(mqttBuf.topic, "%s%s", ESP_MQTT_CLIENT_PUBBLISH, node_id);
    mqttBuf.topic_len = strlen(mqttBuf.topic);
    sprintf(mqttBuf.data, "{\"status\":\"%s\"}", status);
    mqttBuf.data_len = strlen(mqttBuf.data);
    xQueueSend(xQueuePublish, &mqttBuf, 0);
}

/**
 * @brief      Close HTTP firmware Server connection on error.
 *
 */
static void errHTTPclient_deinit(void)
{
...
//static size_t app_firmware_download(const char *url, const uint8_t *node_mac_addr)
{
    ...
    total_size = esp_http_client_fetch_headers(HTTPclient);
    if (total_size <= 0)
    {
